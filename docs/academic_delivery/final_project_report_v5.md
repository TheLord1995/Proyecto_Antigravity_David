...
**Documento de Entrega Final - Versión 5.0 (Optimizado para Evaluación Académica)**  
...
3. **`MonteCarlo`:** Motor estocástico que realiza simulaciones parametrizables de remuestreo (Bootstrap y Shuffle, configurables con un valor por defecto de 1,000 iteraciones) para verificar la supervivencia del algoritmo y el riesgo de ruina (`risk_of_ruin_pct`).
4. **`BacktestValidator`:** Evaluador lógico de políticas de aceptación de estrategias.
5. **`RiskEngine`:** El guardián determinista que aplica las 6 reglas inquebrantables de seguridad operativa a nivel de simulación (R1–R6).
...
5. **Simulación Monte Carlo:** Se computan iteraciones aleatorias de Bootstrap y Shuffle (configurable, predeterminado en 1,000 iteraciones) para obtener la Probabilidad de Ruina y el peor Drawdown proyectado.
...
| :--- | :---: | :--- |
| **Tests ejecutados** | 121 | Ejecutados de forma automatizada mediante pytest. |
| **Tests superados** | 121 | 100% de las pruebas superadas con éxito. |
...
| **Monte Carlo** | ✅ Implementado | Motor estocástico para remuestreo (configurable, 1,000 iteraciones por defecto). |
...
```
       ...
       └────────────────────────┘
         ...
         ▼                    ▼
┌─────────────────┐    ┌────────────────┐
│   Director IA   │    │ Asistente Dev  │
│  Antigravity    │    │  (VS Code)     │
└─────────────────┘    └────────────────┘
```

### 6.1. Director IA Antigravity (Antigravity Director)
...
- Integraron el routing híbrido de modelos (con OpenRouter) optimizando costes operativos y disponibilidad de modelos.
...
- **Pruebas automatizadas ejecutadas:** **121 tests** ejecutados con éxito.
- **Pruebas superadas:** **121 tests** superados.
...
- Generación de la suite de 121 pruebas automatizadas de integración.
...
Se presentan a lo largo del documento las capturas y esquemas reales del prototipo para verificar el funcionamiento del sistema:
1. **Figura 1 (Arquitectura general de Antigravity):** Diagrama de bloques que demuestra el flujo unidireccional de datos de la solución y las capas de seguridad (Sección 3.1).
2. **Figura 2 (Dashboard HTML de validación):** Vista detallada de la consola de visualización del recálculo matemático de métricas y la distribución estocástica de Monte Carlo (Sección 5.3).
3. **Figura 3 (Interfaz gráfica de demostración del pipeline):** Flujo de ejecución interactivo GUI que guía la carga de informes e integra visualmente los módulos de la aplicación (Sección 5.4).
4. **Figura 4 (Informe HTML consolidado generado automáticamente por Antigravity):** Reporte compilado de salida que recibe el usuario con las validaciones deterministas e interpretaciones semánticas consolidadas (Sección 5.5).
